...
const READONLY = new Set(['ls', 'cat', 'echo', 'pwd', 'head', 'tail', 'grep', 'wc', 'which', 'diff', 'stat', 'du', 'cd', 'less', 'more', 'file', 'dirname', 'basename', 'uname', 'date', 'printenv', 'type', 'df', 'ps', 'top', 'free', 'uptime', 'whoami', 'id', 'groups', 'history', 'true', 'false', 'sleep', 'test', 'printf', 'Get-Content', 'Get-ChildItem', 'Get-Process', 'Get-Item', 'Select-String', 'Get-Date', 'Write-Output', 'Get-Location'])
...
const realOf = (p) => {
  ...
  try { return realpathSync(abs) } catch { return abs } // 路径尚不存在（新建文件）时回退 resolve
}
...
const catOf = (categories, key) => { const v = categories[key] || 'ask'; return { d: v === 'auto' ? 'allow' : v, c: key } }
...
function classifyCommand(cmd, trustRoots, categories, mode) {
  ...
  // 接通死代码开关（P2 修复）：curl/wget → networkExec，dd → disk（危险子命令已被 DANGER 拦）
  if (name === 'curl' || name === 'wget') return catOf(categories, 'networkExec')
  ...
  ctx.on('approval/request', (req, next) => {
    ...
    try {
      ...
      if (req.toolName === 'write' || req.toolName === 'edit' || isWriteSemantics(call.args)) {
        ...
        if (config.mode === 'aggressive' || inTrust(t, roots)) { record({ tool: req.toolName, target: t, decision: 'allowed-once', category: 'fileEdit' }); return 'allowed-once' }
