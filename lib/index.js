...
const DEFAULTS = { enabled: true, categories: { ...CATEGORY_DEFAULTS }, trustedDirs: [] }
...
function loadConfig() {
  try {
    ...
    return { enabled: DEFAULTS.enabled, categories: { ...CATEGORY_DEFAULTS }, trustedDirs: [] }
  }
}
...
const PROTECTED = [
  ...
  /(^|\s)\/(etc|usr|System|Library|Applications|dev)(\/|\s|$)/,
  /\.(pem|key|p12|pfx)(\s|$)/,
...
const FIREWALL_CATS = new Set(['danger', 'protected', 'delete', 'privilege', 'networkExec', 'gitPush', 'publish', 'disk'])
...
function extractTargets(cmd) {
  ...
  if (redirs) for (const r of redirs) targets.push(r.replace(/(?:^|[\s|;&])(?:[12])?>>?\s*/, ''))
  const tee = cmd.match(/\btee\s+([^\s|;&"']+)/)
  if (tee) targets.push(tee[1])
  const outOpt = cmd.match(/(?:^|\s)-[oO]\s+([^\s|;&"']+)/)
  if (outOpt) targets.push(outOpt[1])
...
function classifyCommand(cmd, trustRoots, categories) {
  const c = cmd.trim()
  ...
  if (targets !== null) {
    ...
    if (name === 'npm' || name === 'pnpm' || name === 'yarn') {
      if (/-g\b|--global/.test(c)) return catOf(categories, 'privilege')
      if (subOf(c) === 'publish') return catOf(categories, 'publish')
    }
  ...
  if (name === 'git') {
    ...
    if (sub === 'push') return catOf(categories, 'gitPush')
  ...
  if (BUILD.has(name)) {
    if (name === 'docker') {
      const sub = subOf(c)
      if (sub === 'push') return catOf(categories, 'publish')
      if (sub === 'run' || sub === 'exec' || sub === 'rmi' || sub === 'rm') return catOf(categories, 'privilege')
...
function classifyBash(cmd, trustRoots, categories) {
  ...
  if (parts === null) return classifyCommand(cmd, trustRoots, categories)
  let worst = { d: 'allow', c: 'readOnly' }
  for (const part of parts) {
    const r = classifyCommand(part, trustRoots, categories)
...
export function apply(ctx) {
  ...
  function saveConfig(patch) {
    ...
    try {
      ...
      writeFileSync(configPath(), JSON.stringify({ enabled: config.enabled, categories: config.categories, trustedDirs: config.trustedDirs }, null, 2))
    } catch (e) {
  ...
  ctx.on('approval/request', (req, next) => {
    ...
    try {
      ...
      if (req.toolName === 'bash' || req.toolName === 'pwsh') {
        ...
        if (!cmd) return next()
        const r = classifyBash(cmd, roots, config.categories)
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    ...
    try {
      if (name === 'bash' || name === 'pwsh') {
        ...
        if (!cmd) return next()
        const r = classifyBash(cmd, roots, config.categories)
        if (!FIREWALL_CATS.has(r.c)) return next()
    ...
    handler: async (req, res) => {
      try {
        if (req.method === 'GET' || req.method === undefined) {
          return writeJson(res, 200, { ok: true, enabled: config.enabled, categories: config.categories, trustedDirs: config.trustedDirs, audit: audit.slice().reverse() })
        }
        if (req.method === 'POST') {
          ...
          saveConfig(patch)
          return writeJson(res, 200, { ok: true, enabled: config.enabled, categories: config.categories, trustedDirs: config.trustedDirs })
        }
