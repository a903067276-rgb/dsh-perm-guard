...
const WRITE_CMDS = ['cp', 'mv', 'install', 'ln', 'mkdir', 'touch', 'sed', 'awk', 'tr', 'dd', 'rm', 'rmdir', 'unlink', 'tar', 'zip', 'unzip', 'gzip', 'gunzip', 'chmod', 'chown', 'Set-Content', 'Add-Content', 'Out-File', 'Copy-Item', 'Move-Item', 'New-Item', 'Remove-Item', 'Tee-Object', 'Export-Csv', 'Export-Clixml', 'Clear-Content', 'Rename-Item', 'Compress-Archive', 'Expand-Archive']
...
function extractTargets(cmd) {
  ...
  if (WRITE_CMDS.includes(name)) {
    ...
    // 修复(2026-08-24 审查)：引号包裹的路径（Windows 用户目录常含空格，如 C:\Users\John Doe\）
    // 原正则 ([^\s]+) 会把 "C:\Users\John Doe\a.txt" 截成半截 "C:\Users\John"，若半截落在
    // 信任目录内会误放行越界写入（违背"只收紧不放宽"）。现支持 '...'/"..." 完整捕获。
...
function classifyCommand(cmd, trustRoots, categories, mode) {
  ...
  if (nameBase === 'curl' || nameBase === 'wget') return catOf(categories, 'networkExec')
  ...
  if (NET_CMDS.has(name) || NET_CMDS.has(nameBase)) return catOf(categories, 'networkExec')
...
/**
 * 取会话事件列表（2026-09-10 修）：
 *   - dsh ≥ 0.1.5：`Session.events` 被移除（只剩 eventAt/snapshotEvents/ownEvents），改用公开的
 *     `snapshotEvents()`（返回冻结数组，未追加时复用同一快照，取用开销低）。
 *   - dsh ≤ 0.1.2：保留旧的 `session.events` 数组路径（best-effort 兼容，见 README 维护策略段）。
 * 两条路都拿不到时返回 null —— 调用方据此回落人工审批（安全默认，绝不放行）。
 */
function sessionEventsOf(session) {
  ...
  if (typeof session.snapshotEvents === 'function') {
    try {
      ...
      console.warn('[perm-guard] snapshotEvents failed: ' + (error && error.message ? error.message : String(error)))
      return null
    }
  }
  return null
...
function lookupCall(req) {
  ...
  const events = sessionEventsOf(req.agent ? req.agent.session : null)
...
// ===== 插件主体 =====
// ── 配置（v0.3.0 起：随 profile 条目 id 走）──
// 0.1.7 起宿主移除了 settings 命名空间的老 API（register/get/update 全没了），配置改为
// 「插件导出 Config schema + profile patch 里按条目 id 存 config」。字段加 .volatile()
// = 设置页改完即时生效、不重挂插件（宿主 schemastery 需 ≥3.18.3）。
// 兼容：~/.dsh/perm-guard.json 继续当兜底来源（老用户手改 JSON 仍热重载）；
// 一旦 patch 里有同名字段，以 patch（= 设置页/官方表单写入的值）为准。
export const Config = z.object({
  enabled: z.boolean().volatile(),
  mode: z.string().volatile(),
  categories: z.dict(z.string(), z.string()).volatile(),
  trustedDirs: z.array(z.string()).volatile(),
  strictHighRisk: z.boolean().volatile(),
})
...
export function apply(ctx, rawConfig) {
  ...
  // 旧 JSON 热重载兜底（2026-08-24 起的行为保留）：外部手改 perm-guard.json 不重启也生效。
  // 坏 JSON/半写状态跳过（readConfigFile 返回 null），不冲掉内存中的好配置。
  try {
    watchFile(configPath(), { interval: 5000 }, () => {
      const next = readConfigFile()
      if (next) { legacy = next; console.log('[perm-guard] legacy config reloaded from disk') }
    })
  } catch (e) {
    console.error('[perm-guard] watch config failed: ' + (e && e.message ? e.message : String(e)))
  }

  // 实时配置视图：每次属性访问都从 volatile 引用现读 → 设置页保存即生效，无需轮询/重启
  const config = {
    get enabled() {
      const v = refValue(cfgIn.enabled)
      return typeof v === 'boolean' ? v : legacy.enabled
    },
    get mode() {
      const v = refValue(cfgIn.mode)
      return v === 'standard' || v === 'aggressive' ? v : legacy.mode
    },
    get categories() {
      const v = refValue(cfgIn.categories)
      return { ...legacy.categories, ...(v && typeof v === 'object' ? v : {}) }
    },
    get trustedDirs() {
      const v = refValue(cfgIn.trustedDirs)
      return Array.isArray(v) ? shapeConfig({ trustedDirs: v }).trustedDirs : legacy.trustedDirs
    },
    get strictHighRisk() {
      const v = refValue(cfgIn.strictHighRisk)
      return typeof v === 'boolean' ? v : legacy.strictHighRisk
    },
  }
  ...
  async function saveConfig(patch) {
    ...
    const current = {
      ...
      trustedDirs: config.trustedDirs,
      strictHighRisk: config.strictHighRisk,
    }
    const next = {
      ...current,
      ...(typeof patch.enabled === 'boolean' ? { enabled: patch.enabled } : {}),
      ...(patch.mode === 'standard' || patch.mode === 'aggressive' ? { mode: patch.mode } : {}),
      categories: { ...current.categories, ...safeCats },
      trustedDirs: Array.isArray(patch.trustedDirs) ? patch.trustedDirs : current.trustedDirs,
      strictHighRisk: typeof patch.strictHighRisk === 'boolean' ? patch.strictHighRisk : current.strictHighRisk
    }
    const settings = ctx.get('settings')
    if (settings && typeof settings.update === 'function') {
      // 0.1.7 路径：写 profile 条目配置（volatile 字段就地生效，不重挂插件）
      try {
    ...
    // 兜底：没有新 settings 服务时仍写 JSON 文件（老宿主行为）
    try {
  ...
  // 路由注册挂在 ctx.effect 里：webServer.register 返回 disposer，插件卸载/重挂时
  // 必须把路由摘掉，否则 0.1.7 插件管理页「停用→再启用」会 duplicate route 直接抛错。
  ...
  }), 'perm-guard: /api/perm-guard/state route')
}
